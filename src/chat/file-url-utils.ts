...
import type { LangRouterAudioFormat } from '../types/langrouter-chat-completions-input';

import { convertUint8ArrayToBase64 } from '@ai-sdk/provider-utils';
import { LANGROUTER_AUDIO_FORMATS } from '../types/langrouter-chat-completions-input';
...
export const MIME_TO_FORMAT: Record<string, LangRouterAudioFormat> = {
  // MP3 variants
  mpeg: 'mp3',
  mp3: 'mp3',
  // WAV variants
...
/**
 * Converts an audio file part to LangRouter's input_audio data format.
 *
 * This function extracts base64-encoded audio data from a file part and
 * normalizes the format to one of the supported LangRouter audio formats.
 *
 ...
 *   suitable for use in LangRouter's `input_audio` field.
 *
 * @throws {Error} When audio is provided as an HTTP/HTTPS URL. LangRouter requires
...
export function getInputAudioData(part: LanguageModelV3FilePart): {
  ...
  // LangRouter's input_audio doesn't support URLs directly
  if (
    isUrl({
      url: fileData,
      protocols: new Set(['http:', 'https:'] as const),
    ...
    throw new Error(
      `Audio files cannot be provided as URLs.\n\n` +
        `LangRouter requires audio to be base64-encoded. Please:\n` +
        `1. Download the audio file locally\n` +
        `2. Read it as a Buffer or Uint8Array\n` +
        `3. Pass it as the data parameter\n\n` +
        `The AI SDK will automatically handle base64 encoding.\n\n` +
        `Learn more: https://langrouter.ai/docs/features/multimodal/audio`,
    );
  }
  ...
  // Normalize format names for LangRouter using MIME type mapping
  const format = MIME_TO_FORMAT[rawFormat];

  if (format === undefined) {
    const supportedList = LANGROUTER_AUDIO_FORMATS.join(', ');
    throw new Error(
      `Unsupported audio format: "${mediaType}"\n\n` +
        `LangRouter supports the following audio formats: ${supportedList}\n\n` +
        `Learn more: https://langrouter.ai/docs/features/multimodal/audio`,
    );
  }
