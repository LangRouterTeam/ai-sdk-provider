...
import type {
  ...
  LangRouterChatCompletionsInput,
} from '../types/langrouter-chat-completions-input';

import { LangRouterProviderOptionsSchema } from '../schemas/provider-metadata';
...
// Type for LangRouter Cache Control following Anthropic's pattern
export type LangRouterCacheControl = { type: 'ephemeral' };

function getCacheControl(
  providerMetadata: SharedV3ProviderMetadata | undefined,
): LangRouterCacheControl | undefined {
  const anthropic = providerMetadata?.anthropic;
  const langrouter = providerMetadata?.langrouter;

  // Allow both cacheControl and cache_control:
  return (langrouter?.cacheControl ??
    langrouter?.cache_control ??
    anthropic?.cacheControl ??
    anthropic?.cache_control) as LangRouterCacheControl | undefined;
}

export function convertToLangRouterChatMessages(
  prompt: LanguageModelV3Prompt,
): LangRouterChatCompletionsInput {
  const messages: LangRouterChatCompletionsInput = [];
    ...
    switch (role) {
            ...
            switch (part.type) {
              ...
              case 'file': {
                ...
                const fileName = String(
                  part.providerOptions?.langrouter?.filename ??
                    part.filename ??
                    '',
                );
      ...
      case 'assistant': {
        ...
        const parsedProviderOptions =
          LangRouterProviderOptionsSchema.safeParse(providerOptions);
        const messageReasoningDetails = parsedProviderOptions.success
          ? parsedProviderOptions.data?.langrouter?.reasoning_details
          : undefined;
        const messageAnnotations = parsedProviderOptions.success
          ? parsedProviderOptions.data?.langrouter?.annotations
          : undefined;
...
function findFirstReasoningDetails(
  ...
  for (const part of content) {
    if (part.type === 'tool-call') {
      const langrouter = part.providerOptions?.langrouter as
        | Record<string, unknown>
        | undefined;
      const details = langrouter?.reasoning_details;
  ...
  for (const part of content) {
    if (part.type === 'reasoning') {
      const parsed = LangRouterProviderOptionsSchema.safeParse(
        part.providerOptions,
      );
      if (
        parsed.success &&
        parsed.data?.langrouter?.reasoning_details &&
        parsed.data.langrouter.reasoning_details.length > 0
      ) {
        return parsed.data.langrouter.reasoning_details;
      }
    }
  }
