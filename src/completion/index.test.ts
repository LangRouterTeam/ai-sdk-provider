...
describe('doGenerate', () => {
  ...
  it('should omit cost from providerMetadata when undefined', async () => {
    ...
    const langrouterMetadata = providerMetadata?.langrouter as {
      provider?: string;
      usage?: { cost?: number };
    };

    expect(langrouterMetadata?.provider).toBe('google');
    expect(langrouterMetadata?.usage?.cost).toBeUndefined();
    expect('cost' in (langrouterMetadata?.usage ?? {})).toBe(false);
  });
  ...
  it('should include cost: 0 in providerMetadata when cost is zero', async () => {
    ...
    expect(langrouterMetadata?.usage?.cost).toBe(0);
  });
  ...
  it('should default provider to empty string when not returned by API', async () => {
    ...
    expect(langrouterMetadata?.provider).toBe('');
  });
  ...
  it('should extract logprobs', async () => {
    ...
    const provider = createLangRouter({ apiKey: 'test-api-key' });
...
describe('doStream', () => {
  ...
  it('should handle both normal cost and upstream inference cost in finish metadata when both are provided', async () => {
    ...
    expect(langrouterUsage?.cost).toBe(0.0025);
  });
    ...
    server.urls['https://api.langrouter.ai/v1/completions']!.response = {
      ...
      chunks: [
        `data: {"error":{"message": "The server had an error processing your request. Sorry about that! You can retry your request, or contact us through our ` +
          `help center at help.langrouter.ai if you keep seeing this error.","type":"server_error","param":null,"code":null}}\n\n`,
        'data: [DONE]\n\n',
      ],
    };
    ...
    expect(await convertReadableStreamToArray(stream)).toStrictEqual([
      {
        ...
        error: {
          message:
            ...
            'help.langrouter.ai if you keep seeing this error.',
          type: 'server_error',
          code: null,
          param: null,
        },
