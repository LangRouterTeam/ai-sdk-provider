 ...
 *
 * The canonical key logic matches the LangRouter API's deduplication exactly
 * (see langrouter-web/packages/llm-interfaces/reasonings/duplicate-tracker.ts):
...
export class ReasoningDetailsDuplicateTracker {
  ...
  private getCanonicalKey(detail: ReasoningDetailUnion): string | null {
    // This logic matches the LangRouter API's deduplication exactly.
    // See: langrouter-web/packages/llm-interfaces/reasonings/duplicate-tracker.ts
    switch (detail.type) {
