...
import { LangRouterImageModel } from './index';
...
describe('LangRouterImageModel', () => {
  describe('provider methods', () => {
    it('should expose imageModel method', () => {
      const provider = createLangRouter({ apiKey: 'test-key' });
    ...
    it('should create an image model instance', () => {
      ...
      expect(model).toBeInstanceOf(LangRouterImageModel);
      expect(model.modelId).toBe('google/gemini-2.5-flash-image');
      expect(model.provider).toBe('langrouter');
  ...
  describe('doGenerate', () => {
    ...
    it('should return warning when n > 1 is requested', async () => {
      ...
      expect(result.warnings).toContainEqual({
        ...
        feature: 'n > 1',
        details:
          'LangRouter image generation returns 1 image per call. Requested 3 images.',
      });
    });
    ...
    it('should apply runtime providerOptions.langrouter to request', async () => {
      const mock = createCapturingMockFetch(TEST_IMAGE_BASE64);
      const provider = createLangRouter({
        apiKey: 'test-key',
        fetch: mock.fetch,
