...
interface ProviderConfig {
  ...
  model: string;
  providerOptions:
    | { langrouter: { reasoning: { effort: string } } }
    | { langrouter: { includeReasoning: boolean } }
    | undefined;
...
const providersWithReasoning: ProviderConfig[] = [
  {
    ...
    providerOptions: { langrouter: { reasoning: { effort: 'medium' } } },
    hasReasoning: true,
  },
  {
    name: 'gemini',
    model: 'google/gemini-3-flash-preview',
    providerOptions: { langrouter: { includeReasoning: true } },
    hasReasoning: true,
  },
];
...
function verifyReasoningDetailsDeduplication(
  ...
  const firstToolCallProviderOptions = toolCallContents[0]?.providerOptions as
    ...
    | undefined;
  const firstReasoningDetails = firstToolCallProviderOptions?.langrouter
  ...
  for (let i = 1; i < toolCallContents.length; i++) {
    const toolCallProviderOptions = toolCallContents[i]?.providerOptions as
      | Record<string, Record<string, unknown>>
      | undefined;
    const reasoningDetails = toolCallProviderOptions?.langrouter
      ?.reasoning_details as Array<unknown> | undefined;
